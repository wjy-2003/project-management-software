...
class CollaborationConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        await sync_to_async(session_manager.update_member_channel)(
            self.session_id, self.member_id, self.channel_name
        )
        ...
        session = await sync_to_async(session_manager.get_session)(self.session_id)
    ...
    async def disconnect(self, close_code):
        ...
        session_destroyed = await sync_to_async(session_manager.remove_member)(
    ...
    async def handle_structure_change(self, data):
        ...
        is_initiator = await sync_to_async(session_manager.is_initiator)(
            self.session_id, self.member_id
        )

        # Check if the member has edit permission
        can_edit = await sync_to_async(session_manager.can_edit)(
            self.session_id, self.member_id
        )
    ...
    async def apply_structure_change(self, data):
        ...
        structure = await sync_to_async(session_manager.get_structure)(self.session_id)
        ...
        try:
            if operation == "create_folder":
                ...
                structure["folders"] = [
                    f for f in structure["folders"] if f["id"] != payload["id"]
                ]
            elif operation == "delete_file":
                structure["files"] = [
                    f for f in structure["files"] if f["id"] != payload["id"]
                ]
    ...
    async def handle_permission_change(self, data):
        ...
        success = await sync_to_async(session_manager.update_member_role)(
    ...
    async def handle_cursor_update(self, data):
        """
        Handle cursor position updates from a user.
        Broadcast to all other members in the same file.
        """
        await self.channel_layer.group_send(
            self.group_name,
            {
    ...
    async def handle_document_change(self, data):
        """
        Handle document content changes from a user.
        Broadcast to all other members editing the same file.
        """
        # Check if the member has edit permission
        can_edit = await sync_to_async(session_manager.can_edit)(
            self.session_id, self.member_id
        ...
        if not can_edit:
            await self.send(
                text_data=json.dumps(
                    {
                        "type": "error",
                        "message": "Permission denied: viewer role cannot edit documents",
                        "timestamp": datetime.now().isoformat(),
                    }
                )
            )
    ...
    async def handle_file_opened(self, data):
        """
        Handle notification that a user opened a file.
        """
        await self.channel_layer.group_send(
            self.group_name,
            {
    ...
    async def handle_file_saved(self, data):
        """
        Handle notification that a user saved a file.
        """
        await self.channel_layer.group_send(
            self.group_name,
            {
    ...
    async def cursor_update(self, event):
        ...
        # Do not send updates to the sender
        if event["user_id"] != self.member_id:
            await self.send(
                text_data=json.dumps(
                    {
                        "type": "cursor_update",
    ...
    async def file_opened(self, event):
        """Broadcast file opened notification"""
        if event["user_id"] != self.member_id:
            await self.send(
                text_data=json.dumps(
                    {
    ...
    async def file_saved(self, event):
        """Broadcast file saved notification"""
        await self.send(
            text_data=json.dumps(
                {
                    "type": "file_saved",

...
class SessionManagerTestCase(TestCase):
    ...
    def test_structure_operations(self):
        ...
        # Add folder
        structure["folders"].append({"id": "f1", "name": "src", "parent_id": None})
...
class CollaborationAPITestCase(TestCase):
    ...
    def test_get_session_api(self):
        ...
        response = self.client.get(f"/api/collaboration/sessions/{session_id}/")
    ...
    def test_update_permission_api(self):
        ...
        response = self.client.post(
            ...
            data={"initiator_id": "alice", "member_id": "bob", "role": "editor"},
            content_type="application/json",
        )
    ...
    def test_permission_denied_for_non_initiator(self):
        ...
        response = self.client.post(
            f"/api/collaboration/sessions/{session_id}/permissions/",
            data={"initiator_id": "bob", "member_id": "charlie", "role": "editor"},
            content_type="application/json",
        )
    ...
    def test_collaboration_home_page_loads(self):
        """GET collaboration home should return 200 and contain key text"""
        response = self.client.get("/api/collaboration/")
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "实时协作")
